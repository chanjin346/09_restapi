package com.google.cqrs.product.query.service;

import com.google.cqrs.common.dto.Pagination;
...
import com.google.cqrs.product.query.dto.response.ProductDTO;
...
import com.google.cqrs.product.query.mapper.ProductMapper;
...
public class ProductQueryService {
  ...
  public ProductDetailResponse getProduct(Long productCode) {
    ...
    /* Mybatis는 매퍼를 통해서 조회결과를 바로 DTO에 매핑할 수 있음
    *  -> JPA 보다 편함(Query Side 장점)*/
    // Optional : Null일 경우와 아닐 경우에 대한 동작을 선택하는
    //            Null 처리 객체

    // 조회 결과가 있으면 ProductDTO 반환
    // 조회 결과가 없을 경우 예외 발생
  ...
  /* 상품 목록 조회 */
  @Transactional(readOnly = true) // 조회만 하겠다 실수로 추가 수정 삭제하는걸 막겠다~
